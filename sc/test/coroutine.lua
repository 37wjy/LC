...
local function startcoroutine(name, key, func, ...)
	co_list = co_list or {}
	local co = coroutine.start(func, ...)
	if name then
		local co_list_by_name = co_list[name] or {}
		if key == nil or co_list_by_name[key] then
			key = "Default_" .. (table.count(co_list_by_name) + 1)
		end
		co_list_by_name[key] = co
		co_list[name] = co_list_by_name
	end
end

local function stopcoroutine(name, key)
	co_list = co_list or {}
	local co_list_by_name
	if name then
		co_list_by_name = co_list[name]
		if co_list_by_name then
			if key then
				if co_list_by_name[key] then
					coroutine.stopwaiting(co_list_by_name[key])
					co_list[name][key] = nil
				end
			else
				for k, co in pairs(co_list_by_name) do
					coroutine.stopwaiting(co)
					co_list[name][k] = nil
				end
				co_list[name] = nil
			end
...
coroutine.stopcoroutine = stopcoroutine
